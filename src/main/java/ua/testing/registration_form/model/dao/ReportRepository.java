...
public interface ReportRepository extends JpaRepository<Report, Long> {
    ...
    @Query("select u from Report u where u.taxpayer.login = :login and "
            + "u.creationTime = :time")
    List <Report>  findByTaxpayerLoginAndTime(
            @Param("login") String taxpayerLogin,
            @Param("time") LocalDateTime createTime);

    @Query("select u from Report u where u.taxpayer.login = :login and "
            + "u.creationTime = :time")
    Report  find_1_ByTaxpayerLoginAndTime(
            @Param("login") String taxpayerLogin,
            @Param("time") LocalDateTime createTime);

    @Modifying
    @Query("update Report u set u.assessed = true, u.taxofficer = "
            + ":taxofficer where u.id = :id")
    ...
    @Modifying
    @Query("update Report u set u.accepted = true, u.assessed = true, "
            + "u.acceptTime = :acceptTime, u.taxofficer = :taxofficer "
            + "where u.id = :reportId")
